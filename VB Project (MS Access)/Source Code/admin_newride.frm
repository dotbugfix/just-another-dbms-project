...
sqlquery = "select count(*) as num from rides where name='" & RideName.Text & "'"
rsRide.Open sqlquery, connRide

If (rsRide.Fields("num").Value > 0) Then
    MsgBox "A ride with this name already exists in the database!", vbOKOnly, "Admin Landing"
Else
rsRide.Close
...
Private Sub Cost_Adults_KeyPress(KeyAscii As Integer)
...
Private Sub Cost_Kids_KeyPress(KeyAscii As Integer)
...
MsgBox "Use this function with care. Each ride contributes to the monthly expenses of the resort. Improper values may lead to an absurd financial report!", vbExclamation, "Demonstration"
...
Private Sub Op_Cost_KeyPress(KeyAscii As Integer)
